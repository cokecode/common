...
/*
 * Usage:
 *  argv[0] <interface> <src-ip> <dst-ip> <seq> <ack> <flags>
 * Eg,
 *  argv[0] eth1 192.168.100.1 192.168.100.20 586569568 6855765 SA
 */
int
main (int argc, char **argv)
{
  ...
  const int on = 1;
  char *interface, *target, *src_ip, *dst_ip, *flags;
  struct ip iphdr;
  ...
  if (argc < 9) {
    printf("Usage:\n");
    printf("  %s <interface> <src-ip> <src-port> <dst-ip> <dst-port> <seq> <ack> <flags>\n", argv[0]);
    printf("E.g.,\n");
    printf("  %s eth1 192.168.100.1 12345 192.168.100.20 8080 586569568 6855765 SA\n", argv[0]);
    exit(1);
  }
  ...
  seq = strtoul(argv[6], NULL, 0);
  ack = strtoul(argv[7], NULL, 0);
  ...
  for (i = 0; flags[i] != '\0'; i++) {
    switch (flags[i]) {
      case 'F': tcp_flags[0] = 1; break;  // FIN
      case 'S': tcp_flags[1] = 1; break;  // SYN
      case 'R': tcp_flags[2] = 1; break;  // RST
      case 'P': tcp_flags[3] = 1; break;  // PSH
      case 'A': tcp_flags[4] = 1; break;  // ACK
      case 'U': tcp_flags[5] = 1; break;  // URG
      case 'E': tcp_flags[6] = 1; break;  // ECE
      case 'C': tcp_flags[7] = 1; break;  // CWR
      default: break;
    }
  }
...
// vi:ts=2:sw=2:ai:cindent:et:bg=dark
